...
/**
 * Replacement for <code>java.util.ZipInputStream</code>.
 *
 ...
 *
 * <p>It doesn't extend <code>java.util.zip.ZipInputStream</code> as it would
 * have to reimplement all methods anyway.  Like
 * <code>java.util.ZipInputStream</code>, it uses RandomAccessFile under the
 ...
 * <code>java.util.zip.ZipInputStream</code>, with a couple of exceptions:
 *
 * <ul>
...
public class ZipInputStream {
     ...
     */
    private final RandomBuffer archive;
     ...
     */
    public ZipInputStream(InputStream f) throws IOException {
        this(f, null);
    }
     ...
     * @throws IOException if an error occurs while reading the file.
     */
    public ZipInputStream(InputStream is, String encoding) throws IOException {
     ...
     * @throws IOException if an error occurs while reading the file.
     */
    public ZipInputStream(InputStream is, String encoding, boolean useUnicodeExtraFields)
        throws IOException {
        this.archiveName = null;
        this.encoding = encoding;
        this.zipEncoding = ZipEncodingHelper.getZipEncoding(encoding);
        this.useUnicodeExtraFields = useUnicodeExtraFields;
        this.archive = RandomBuffer.getRandomBuffer(is);
     ...
     */
    public static void closeQuietly(ZipInputStream zipfile) {
        if (zipfile != null) {
            try {
                zipfile.close();
            } catch (IOException e) {
    ...
    protected void finalize() throws Throwable {
        try {
            if (!closed) {
                System.err.println("Cleaning up unclosed ZipInputStream for archive "
                                   + archiveName);
                close();
            }
        } finally {
    ...
    private void
        ...
        archive.read(cdExtraData);
        ze.setCentralDirectoryExtra(cdExtraData);

        setSizesAndOffsetFromZip64Extra(ze, offset, diskStart);

        byte[] comment = new byte[commentLen];
        archive.read(comment);
    ...
    private void positionAtCentralDirectory()
        ...
        boolean searchedForZip64EOCD =
            archive.pos() > ZIP64_EOCDL_LENGTH;
        if (searchedForZip64EOCD) {
            archive.seek(archive.pos() - ZIP64_EOCDL_LENGTH);
            archive.read(WORD_BUF);
    ...
    private void skipBytes(final int count) throws IOException {
        ...
        while (totalSkipped < count) {
            int skippedNow = (int) archive.skip(count - totalSkipped);
            if (skippedNow <= 0) {
    ...
    private void resolveLocalFileHeaderData(Map<ZipEntry, NameAndComment>
        ...
        for (Map.Entry<ZipEntry, OffsetEntry> ent : origMap.entrySet()) {
            ...
            decoder.reset();
            try {
                decoder.decode(ByteBuffer.wrap(localExtraData));
