...
public class EpubWriter {
	...
	private void writeResource(Resource resource, ZipOutputStream resultStream)
		...
		try {
			ZipEntry ze = new ZipEntry("OEBPS/" + resource.getHref());
			if (resource.getCompressMethod() == ZipEntry.STORED) {
				ze.setMethod(resource.getCompressMethod());
				CRC32 checksum = new CRC32();
				checksum.update(resource.getData());
				ze.setCrc(checksum.getValue());
				ze.setSize(resource.getSize());
			}
