...
class PackageDocumentMetadataReader extends PackageDocumentBase {
	...
	public static Metadata readMetadata(Document packageDocument, Resources resources) {
		...
		if(metadataElement == null) {
			Logger.getLogger(PackageDocumentMetadataReader.class.getName()).log(Level.WARNING, "Package does not contain element " + OPFTags.metadata);
			return result;
		}
	...
	private static Map<QName, String> readOtherProperties(Element metadataElement) {
		...
		for (int i = 0; i < metaTags.getLength(); i++) {
			...
			else if (content != null && nameAttr != null) {
				String name = nameAttr.getNodeValue();
				String value = content.getNodeValue();
				result.put(new QName(name), value);
			}
		}
		NodeList nl = metadataElement.getElementsByTagName("cover");
		if (nl != null && nl.getLength() > 0) {
			result.put(new QName("cover"), nl.item(0).getTextContent());
		}
		nl = metadataElement.getElementsByTagName("order");
		if (nl != null && nl.getLength() > 0) {
			result.put(new QName("order"), nl.item(0).getTextContent());
		}
		nl = metadataElement.getElementsByTagName("kt");
		if (nl != null && nl.getLength() > 0) {
			result.put(new QName("kt"), nl.item(0).getTextContent());
		}
		nl = metadataElement.getElementsByTagName("version");
		if (nl != null && nl.getLength() > 0) {
			result.put(new QName("version"), nl.item(0).getTextContent());
		}
	...
	private static List<Identifier> readIdentifiers(Element metadataElement) {
		...
		if(identifierElements.getLength() == 0) {
			Logger.getLogger(PackageDocumentMetadataReader.class.getName()).log(Level.WARNING, "Package does not contain element " + DCTags.identifier);
