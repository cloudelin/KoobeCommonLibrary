...
//	}
	...
	private static Resources readManifest(Document packageDocument, String packageHref,
		...
		if(manifestElement == null) {
			Logger.getLogger(PackageDocumentReader.class.getName()).log(Level.WARNING, "Package document does not contain element " + OPFTags.manifest);
			return result;
		}
		...
		for(int i = 0; i < itemElements.getLength(); i++) {
			...
			if(resource == null) {
				int x = href.lastIndexOf('.');
				if (x != -1) {
					String href2 = href.substring(0, x);
					resource = resources.remove(href2);
				}
				if (resource == null) {
					Logger.getLogger(PackageDocumentReader.class.getName()).log(Level.WARNING, "resource with href '" + href + "' not found");
					continue;
				}
			}
			resource.setId(id);
	...
	private static void readGuide(Document packageDocument,
		...
		for (int i = 0; i < guideReferences.getLength(); i++) {
			...
			if (resource == null) {
				Logger.getLogger(PackageDocumentReader.class.getName()).log(Level.WARNING, "Guide is referencing resource with href " + resourceHref + " which could not be found");
				continue;
			}
			String type = DOMUtil.getAttribute(referenceElement, NAMESPACE_OPF, OPFAttributes.type);
			if (StringUtil.isBlank(type)) {
				Logger.getLogger(PackageDocumentReader.class.getName()).log(Level.WARNING, "Guide is referencing resource with href " + resourceHref + " which is missing the 'type' attribute");
				continue;
			}
	...
	private static Spine readSpine(Document packageDocument, EpubReader epubReader, Resources resources, Map<String, String> idMapping) {
		...
		if (spineElement == null) {
			Logger.getLogger(PackageDocumentReader.class.getName()).log(Level.WARNING, "Element " + OPFTags.spine + " not found in package document, generating one automatically");
		...
		for(int i = 0; i < spineNodes.getLength(); i++) {
			...
			if(StringUtil.isBlank(itemref)) {
				Logger.getLogger(PackageDocumentReader.class.getName()).log(Level.WARNING, "itemref with missing or empty idref"); // XXX
				continue;
			}
			...
			if(resource == null) {
				Logger.getLogger(PackageDocumentReader.class.getName()).log(Level.WARNING, "resource with id \'" + id + "\' not found");
				continue;
			}
	...
	private static Resource findTableOfContentsResource(Element spineElement, Resources resources) {
		...
		Collection<Resource> colResource = resources.getAll();
		for (Resource res : colResource) {
			String properties = res.getProperties();
			if (properties != null && properties.equalsIgnoreCase(OPFValues.nav)) {
				tocResource = res;
				return tocResource;
			}
		}
		
		// find possible ncx resource
		...
		if (tocResource == null) {
			Logger.getLogger(PackageDocumentReader.class.getName()).log(Level.WARNING, "Could not find table of contents resource. Tried resource with id '" + tocResourceId + "', " + Constants.DEFAULT_TOC_ID + ", " + Constants.DEFAULT_TOC_ID.toUpperCase() + " and any NCX resource.");
		}
		return tocResource;
	}
	...
	private static void readCover(Document packageDocument, Book book) {
		...
		for (String coverHref: coverHrefs) {
			...
			if (resource == null) {
				Logger.getLogger(PackageDocumentReader.class.getName()).log(Level.WARNING, "Cover resource " + coverHref + " not found");
				continue;
			}
