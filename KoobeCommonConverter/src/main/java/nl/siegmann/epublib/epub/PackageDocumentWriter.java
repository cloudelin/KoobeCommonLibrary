...
//		writeCoverResources(book, serializer);
	...
	private static void writeItem(Book book, Resource resource, XmlSerializer serializer) throws IllegalArgumentException, IllegalStateException, IOException {
		...
		if(StringUtil.isBlank(resource.getId())) {
			Logger.getLogger(PackageDocumentWriter.class.getName()).log(Level.SEVERE, "resource id must not be empty (href: " + resource.getHref() + ", mediatype:" + resource.getMediaType() + ")");
			return;
		}
		if(StringUtil.isBlank(resource.getHref())) {
			Logger.getLogger(PackageDocumentWriter.class.getName()).log(Level.SEVERE, "resource href must not be empty (id: " + resource.getId() + ", mediatype:" + resource.getMediaType() + ")");
			return;
		}
		if(resource.getMediaType() == null) {
			Logger.getLogger(PackageDocumentWriter.class.getName()).log(Level.SEVERE, "resource mediatype must not be empty (id: " + resource.getId() + ", href:" + resource.getHref() + ")");
			return;
		}
		serializer.startTag(NAMESPACE_OPF, OPFTags.item);
