...
public class NCXDocument {
	...
	private interface NCXTags {
		...
		String navHtml5 = "nav";
		String navAnchor = "a";
		String navOrderList = "ol";
		String navListItem = "li";
	}
	...
	private interface NCXAttributeValues {
		...
		/// epub3
		String type_toc = "toc";
		String type_landmarks = "landmarks";
	}
	
	public static Resource read(Book book, EpubReader epubReader) {
		Resource ncxResource = null;
		if(book.getSpine().getTocResource() == null) {
			Logger.getLogger(NCXDocument.class.getName()).log(Level.WARNING, "Book does not contain a table of contents file");
			return ncxResource;
		}
		try {
			...
			// epub3 only
				...
				List<TOCReference> tocs = readTOCReferencesbyNavEpub3(ncxDocument.getDocumentElement(), book);
				List<TOCReference> tocs_spread = spreadTOCReferences(tocs);
				tableOfContents = new TableOfContents(tocs_spread);
				//tableOfContents = new TableOfContents(readTOCReferencesbyNavEpub3(ncxDocument.getDocumentElement(), book));				
			}
	...
	// Spread out a toc reference tree into a raw list
	private static List<TOCReference> spreadTOCReferences(List<TOCReference> i_tocs) {
		if (i_tocs == null) {
			return new ArrayList<TOCReference>();
		}
		List<TOCReference> result = new ArrayList<TOCReference>(i_tocs.size());
		traverseTOCReferences(i_tocs, result);
		return result;
	}
	private static void traverseTOCReferences(List<TOCReference> i_tocs, List<TOCReference> o_tocs) {
		if (i_tocs == null || o_tocs == null) {
			return;
		}
		for (int i=0; i<i_tocs.size(); i++) {
			TOCReference i_ref = i_tocs.get(i);
			o_tocs.add(new TOCReference(i_ref.getTitle(), i_ref.getResource(), i_ref.getFragmentId()));
			//traverse chidren
			traverseTOCReferences(i_ref.getChildren(), o_tocs);
		}
	}
	
	// for epub3
	private static List<TOCReference> readTOCReferencesbyNavEpub3(Element documentElement, Book book) {
		if(documentElement == null) {
			return new ArrayList<TOCReference>();
		}
		
		List<TOCReference> result = new ArrayList<TOCReference>();
		Element navEpub3Element = null;
		
		// find <nav epub:type="toc"> as navEpub3Element
		NodeList navElements = documentElement.getElementsByTagName(NCXTags.navHtml5);			
		if (navElements != null) {
			for (int i=0; i<navElements.getLength(); i++) {
				Node node = navElements.item(i);
				String type = DOMUtil.getAttribute((Element)node, NAMESPACE_EPUB, NCXAttributes.epub_type);
				if (type != null && type.equalsIgnoreCase(NCXAttributeValues.type_toc)) {
					navEpub3Element = (Element)node;
					break;
				}
			}
		}
		
		// find the uniqe child <ol>
		if (navEpub3Element != null) {
			Element olElement = DOMUtil.getFirstElementByTagNameNS(navEpub3Element, "*", NCXTags.navOrderList);
			if (olElement != null) {
				result = readTOCReferencebyOrderListEpub3(olElement, book);
/* example
<nav epub:type="toc">
	<ol>
		<li>
			<a href=".xhtml#">Text Label</a>
		</li>
		<li>
			<a href=".xhtml#">Text Label</a>
			<ol>
...
*/
	...
	// for epub3
	private static List<TOCReference> readTOCReferencebyOrderListEpub3(Element olElement, Book book) {
		if(olElement == null) {
			return new ArrayList<TOCReference>();
		}
		...
		// for each <li>
		NodeList items = olElement.getChildNodes();
		if (items != null) {
			for(int i = 0; i < items.getLength(); i++) {
				Node node = items.item(i);
				if (node.getNodeType() == Document.ELEMENT_NODE
					&& node.getLocalName().equals(NCXTags.navListItem))
				{
					TOCReference tocReference = readTOCReferencebyListItemEpub3((Element) node, book);
	...
	// for epub3s
	private static TOCReference readTOCReferencebyListItemEpub3(Element liElement, Book book) {
		if (liElement == null) {
			return null;
		}
		...
		// find the unique <a href> child
		Element anchorElement = DOMUtil.getFirstElementByTagNameNS(liElement, "*", NCXTags.navAnchor);
		if (anchorElement != null) {
			String label = anchorElement.getTextContent();
			label = label.trim();
			label = label.replaceAll("[\t\r\n]", "");
			String reference = anchorElement.getAttribute(NCXAttributes.href);
			...
			//find resource by partial path (ex: /abc.html)
			if (resource == null) {
				resource = book.getResources().findResourceByPartialPath(href);
			}		
			if (resource == null) {
				Logger.getLogger(NCXDocument.class.getName()).log(Level.WARNING, "Resource with href " + href + " in NCX document not found");
				resource = new Resource(href);
			}
		
			// new toc regarding <a href>
			result = new TOCReference(label, resource, fragmentId);
		
			// find the unique <ol> as the root of children toc referencess
			Element olElement = DOMUtil.getFirstElementByTagNameNS(liElement, "*", NCXTags.navOrderList);
			if (olElement != null) {
				result.setChildren(readTOCReferencebyOrderListEpub3(olElement, book));
			}
		}
